                    ...
                    <div class="h-100">
                        <div class="card p-2 module mb-2">
                            ...
                            <a style="display:flex" id="moduleLink" class="stretched-link w-100 {{ $disabled }}" {!! $disabled ? '' : 'href='.route('explore.module', ['module_id' => $module->id]) !!}>
                                <img src="{{ Storage::url('content/Flower-'. $module->numDaysCompleted .'.svg') }}" alt="Icon" style="width:50px; height:50px; margin-right:10px;">
                                {{ $module->name }} <br> {{ $module->numDaysCompleted }}/{{ $module->totalDays }} days completed
                            </a>
